...
use crate::modules::users::adapters::outbound::persistence::UserRow;
...
impl UserRepositoryPort for PgUserRepository {
    async fn save(&self, user: &User) -> Result<(), AppError> {
        ...
        sqlx::query(
            "INSERT INTO users (id, name, email, email_verified_at, phone, phone_verified_at, password_hash, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)",
        )
    ...
    async fn find_by_email(&self, email: &Email) -> Result<Option<User>, AppError> {
        ...
        let row = sqlx::query_as::<_, UserRow> ("SELECT id, name, email, email_verified_at, phone, phone_verified_at, password_hash, created_at, updated_at FROM users WHERE email = $1")
